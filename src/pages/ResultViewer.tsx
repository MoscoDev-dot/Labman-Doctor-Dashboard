            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-md border border-border p-3">
                  ...
                  <p
                    className={`text-2xl font-bold ${result.flag === "Critical" ? "text-destructive" : result.flag === "Abnormal" ? "text-foreground" : "text-foreground"}`}
                  >
                    {result.run1}
              ...
              <div className="flex items-center gap-4 text-xs text-muted-foreground pt-2 border-t border-border">
                <span>
                  Collected: {new Date(result.dateTime).toLocaleString()}
                </span>
                {result.releasedAt && (
                  <span>
                    Released: {new Date(result.releasedAt).toLocaleString()}
                  </span>
                )}
                <Badge variant="secondary" className="text-xs">
                  {result.status}
